...
function payload(f:FormData){
  ...
  const sortOrder=Number(String(f.get("sort_order")||"0"));
  ...
  if(!Number.isInteger(sortOrder)||sortOrder<0) throw new Error("Sort order tidak valid.");
  return {name,slug,category,price,
    ...
    screenshots:jsonList(f.get("screenshots")),
    features:jsonList(f.get("features")),
    tech_stack:jsonList(f.get("tech_stack")),
